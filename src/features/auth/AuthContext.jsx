...
export function AuthProvider({ children }) {
  ...
  async function signUp({ email, password, name, phone, role }) {
    // name/phone/role travel as user metadata; a database trigger
    // (supabase/02_handle_new_user_trigger.sql) reads it and creates the
    // profiles row server-side — no client-side insert, no RLS timing issues.
